...
class Config:
    # 基础设置
    USE_COUNT = 1000
    CLONE_BASE_DIR = 'third_package_source'
...
def normalize_github_url(url: str) -> Optional[Tuple[str, str]]:
...
def extract_github_info(project_urls: Dict[str, str]) -> Optional[Tuple[str, str]]:
...
def clone_or_update_repo(owner: str, repo: str) -> Optional[str]:
    """克隆或更新GitHub仓库到本地"""
...
def search_keywords_in_local_repo(repo_path: str) -> List[Dict[str, Any]]:
    ...
    remaining_keywords = set(Config.KEYWORD_TO_EXTENSION.keys())
    found_keywords_global = set()
...
def process_single_package(package: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """处理单个包的逻辑"""
