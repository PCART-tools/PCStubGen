from __future__ import annotations

from pathlib import Path

from loguru import logger

from .c_extension.completer import CExtensionCompleter
from .completion_models import (
    SignatureCompletionContext,
    SignatureCompletionResult,
    SignatureCompletionSummary,
    UnsupportedSignatureCompletion,
)
from .minimal_completer import MinimalCompleter
from .pybind11.completer import Pybind11Completer


class SignatureCompleter:
    """编排 completer 执行并在失败时回退到最小签名。"""

    def __init__(self, compilation_database: Path) -> None:
        self._c_extension_completer = CExtensionCompleter(compilation_database)
        self._pybind11_completer = Pybind11Completer()
        self._minimal_completer = MinimalCompleter()
        self.summary = SignatureCompletionSummary()

    def reset_summary(self) -> None:
        """重置本轮补全统计。"""
        self.summary = SignatureCompletionSummary()

    def match(
        self,
        member: object,
        owner_class: type | None = None,
    ) -> bool:
        """判断运行时对象是否匹配任一签名补全 completer。"""
        return (
            self._c_extension_completer.match(member, owner_class)
            or self._pybind11_completer.match(member, owner_class)
        )

    def complete(self, context: SignatureCompletionContext) -> SignatureCompletionResult:
        """补全单个 callable。"""
        reason = "函数不属于受支持的签名补全来源。"

        if self._c_extension_completer.match(context.member, context.owner_class):
            completer = "c_extension"
            try:
                result = self._c_extension_completer.get(context)
            except UnsupportedSignatureCompletion as ex:
                reason = f"{ex!r}"
            except Exception as ex:
                return self._fallback_to_minimal(context, completer, f"{ex!r}")
            else:
                return self._complete_success(context, completer, result)

        if self._pybind11_completer.match(context.member, context.owner_class):
            completer = "pybind11"
            try:
                result = self._pybind11_completer.get(context)
            except UnsupportedSignatureCompletion as ex:
                reason = f"{ex!r}"
            except Exception as ex:
                return self._fallback_to_minimal(context, completer, f"{ex!r}")
            else:
                return self._complete_success(context, completer, result)

        raise UnsupportedSignatureCompletion(reason)

    def _complete_success(
        self,
        context: SignatureCompletionContext,
        completer: str,
        result: SignatureCompletionResult,
    ) -> SignatureCompletionResult:
        """记录 completer 成功补全结果。"""
        self.summary.total += 1
        if completer == "c_extension":
            self.summary.c_extension += 1
        elif completer == "pybind11":
            self.summary.pybind11 += 1
        _log_success(context, completer)
        return result

    def _fallback_to_minimal(
        self,
        context: SignatureCompletionContext,
        completer: str,
        reason: str,
    ) -> SignatureCompletionResult:
        """在 completer 真实失败时回退到最小签名。"""
        self.summary.total += 1
        self.summary.failed += 1
        result = self._minimal_completer.get(context)
        _log_failure(context, completer, reason)
        return result


def _log_success(context: SignatureCompletionContext, completer: str) -> None:
    logger.info(
        "补全成功, completer: {}, module: {}, func: {}, owner_class: {}",
        completer,
        context.module_name,
        context.func_name,
        context.owner_class,
    )


def _log_failure(
    context: SignatureCompletionContext,
    completer: str,
    reason: str,
) -> None:
    logger.warning(
        "补全失败, completer: {}, module: {}, func: {}, owner_class: {}, reason: {}",
        completer,
        context.module_name,
        context.func_name,
        context.owner_class,
        reason,
    )
