...
class CExtensionProvider:
    ...
    def support(handle: object) -> bool:
        ...
        return (runtime.is_c_extension_module_function(handle) or
                runtime.is_c_extension_instance_method(handle) or
                runtime.is_c_extension_static_method(handle) or
                runtime.is_c_extension_static_method(handle))
    ...
    def get(self, context: SignatureCompletionContext) -> tuple[list[Signature], str]:
        """为单个函数执行 C 源码签名推断。"""
        func_cursor, flags = self.get_func_cursor_and_flags(context.handle)
