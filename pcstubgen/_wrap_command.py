...
def ensure_wrap_programs_available() -> None:
    """检查 wrap 命令依赖的外部程序。"""
    missing_programs = [
        program
...
def run_wrap_command(command: Sequence[str], output: Path) -> int:
    """执行包装后的构建命令并返回退出码。"""
    env = os.environ.copy()
    ...
    try:
        ...
        raise RuntimeError(f"执行 wrap 命令失败: {ex}") from ex
...
def _wrap_command(
    ...
    """
    try:
        ensure_wrap_programs_available()
        return_code = run_wrap_command(command, output)
