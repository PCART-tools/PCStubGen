...
def test_extract_cursor_source_text_raises_when_extent_start_file_is_missing() -> None:
    with pytest.raises(RuntimeError, match="缺少起止文件信息"):
        cursor_utils_module.source_range_get_text(
            _FakeSourceRange(
...
def test_extract_cursor_source_text_raises_for_cross_file_extent(tmp_path: Path) -> None:
    first = tmp_path / "first.c"
    second = tmp_path / "second.c"
    first.write_text("abc", encoding="utf-8")
    second.write_text("def", encoding="utf-8")

    with pytest.raises(RuntimeError, match="跨越多个文件"):
        cursor_utils_module.source_range_get_text(
            _FakeSourceRange(
...
def test_extract_cursor_source_text_raises_when_file_read_fails(tmp_path: Path) -> None:
    missing = tmp_path / "missing_extent_text.c"

    with pytest.raises(RuntimeError, match="源码范围读取失败"):
        cursor_utils_module.source_range_get_text(
            _FakeSourceRange(
