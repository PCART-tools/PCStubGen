...
_single_transparent_cursor_kinds = {
    # 没有暴露给python libclang的表达式，底层可能是不同的clang cursor kind
...
def _is_integer_literal_zero(cursor: Cursor) -> bool:
    """
...
def _is_nullptr_or_zero(node: Cursor) -> bool:
    """识别 `0` / `NULL` / `nullptr`。"""
    if node.kind in _nullptr_cursor_kinds:
        return True
    if _is_integer_literal_zero(node):
        return True
    return False


def _diagnostic_severity_to_str(severity: int) -> str:
...
def _diagnostic_to_str(diagnostic: Diagnostic) -> str:
    """将单条 clang diagnostic 格式化为稳定的一行文本。"""
    severity = _diagnostic_severity_to_str(diagnostic.severity)
...
def _format_diagnostics_message(
    ...
    lines.extend(f"    {_diagnostic_to_str(diagnostic)}" for diagnostic in diagnostics)
    return "\n".join(lines)
...
def _explode_Py_BuildValue_format_string(format_text: str) -> list[str]:
    """
...
def _infer_return_type_from_Py_BuildValue_format(format_text: str) -> str:
    """根据 `Py_BuildValue` 格式串估算返回类型。"""
    markers = _explode_Py_BuildValue_format_string(format_text)
    if not markers:
        return "None"
...
def _find_first_call_name(node: Cursor) -> str | None:
...
def _collect_identifier_literal_tokens(node: Cursor) -> list[str]:
...
def _is_parameter_parser_call(call_name: str) -> bool:
...
def _resolve_format_index(call_name: str, meth_flags: list[str]) -> tuple[int, int]:
    """
...
def _explode_format_string(format_text: str) -> list[str]:
    """
...
def _split_required_optional(markers: list[str]) -> tuple[list[str], list[str]]:
...
def _collect_call_tokens(call_node: Cursor) -> list[str]:
    """
...
def _apply_method_flags(signature: ExtractedSignature, meth_flags: list[str]) -> ExtractedSignature:
    """
    根据 `METH_*` 规则修正首参语义。
...
def _build_module_lookup_names(module_name: str) -> set[str]:
    """构建模块别名集合，仅保留完整名与叶子名。"""
...
def _get_init_value(name: str, func_cursor: Cursor) -> str | None:
    """从局部变量定义中提取默认值字面表达式。"""
...
def _find_format_string(func_cursor: Cursor, format_var_name: str) -> str | None:
    """回溯查找格式串变量对应的字符串字面量。"""
    for node in _walk_cursor(func_cursor):
...
def _normalize_parser_type(raw_type: str) -> str:
...
def _merge_discovered_functions(
    ...
    for function in functions:
        _add_discovered_function(target, function, module_name=module_name)
...
def _resolve_Py_BuildValue_format_token(*, func_cursor: Cursor, token: str) -> str | None:
    """解析 `Py_BuildValue` 的格式串 token。"""
    if '"' in token:
        return _strip_string_literal_quotes(token)
    if _looks_like_identifier(token):
        return _find_format_string(func_cursor=func_cursor, format_var_name=token)
    return None


def _parse_parser_args(args_text: str) -> list[ExtractedArgument]:
    """
    解析 parser 风格的参数文本。
    ...
    for raw_part in parts:
        ...
        if " " in before_default:
            ...
            if _looks_like_identifier(name_candidate):
                name = name_candidate
                type_name = _normalize_parser_type(type_candidate)
            else:
                name = f"arg{len(result) + 1}"
                type_name = _normalize_parser_type(before_default)
        else:
            name = before_default if _looks_like_identifier(before_default) else f"arg{len(result) + 1}"
...
def _parse_format_token(func_cursor: Cursor, token: str) -> list[str] | None:
    """将格式串 token 解析为 marker 列表，支持字面量与变量两种来源。"""
    if token == "F_INT_PYFMT":
        return ["F_INT_PYFMT"]
    if '"' in token:
        text = _strip_string_literal_quotes(token)
        return _explode_format_string(text)
    if _looks_like_identifier(token):
        text = _find_format_string(func_cursor=func_cursor, format_var_name=token)
        if text:
            return _explode_format_string(text)
    return None


def _infer_return_type_from_Py_BuildValue(return_stmt: Cursor, func_cursor: Cursor) -> str:
    """从 `Py_BuildValue` 的格式串推断返回类型。"""
    tokens = _collect_identifier_literal_tokens(return_stmt)
    if "Py_BuildValue" not in tokens:
        return "object"

    call_idx = tokens.index("Py_BuildValue")
    for token in tokens[call_idx + 1:]:
        format_text = _resolve_Py_BuildValue_format_token(func_cursor=func_cursor, token=token)
        if format_text is None:
            continue
        return _infer_return_type_from_Py_BuildValue_format(format_text)
    return "object"
...
def _infer_return_type_from_call(
    ...
    if call_name == "Py_BuildValue":
        return _infer_return_type_from_Py_BuildValue(return_stmt=return_stmt, func_cursor=func_cursor)

    if call_name == "Py_NewRef":
        token_set = set(_collect_identifier_literal_tokens(return_stmt))
...
def _extract_parser_signatures(node: Cursor) -> list[ExtractedSignature]:
    ...
    for text in literals:
        ...
        args = _parse_parser_args(args_part)
        if args:
...
def _set_token_params(
    ...
    if not _is_parameter_parser_call(call_name):
        return None

    format_idx, offset = _resolve_format_index(call_name=call_name, meth_flags=meth_flags)
    if format_idx < 0:
        return None
    ...
    for idx in range(format_idx, len(token_list)):
        parsed = _parse_format_token(func_cursor, token_list[idx])
        if parsed is None:
            continue
    ...
    required, optional = _split_required_optional(format_markers)
    ...
    for marker, is_optional in ([(m, False) for m in required] + [(m, True) for m in optional]):
        ...
        default_value = _get_init_value(name=name, func_cursor=func_cursor) if is_optional else None
        kind = "keyword_only" if kw_only else "positional_or_keyword"
        result.append(
            ExtractedArgument(
                name=name,
...
def _infer_return_type_from_return_stmt(return_stmt: Cursor, func_cursor: Cursor) -> str | None:
    ...
    call_name = _find_first_call_name(return_stmt)
    if call_name is None:
        return None
    return _infer_return_type_from_call(
        call_name=call_name,
...
def _signature_from_param_decls(func_cursor: Cursor) -> ExtractedSignature:
    """回退方案：直接从 C 形参声明推断签名。"""
    ...
    for node in func_cursor.get_children():
        ...
        args.append(
            ExtractedArgument(
                name=node.spelling,
                type_name=_normalize_c_type(node.type.spelling),
                default_value=None,
            )
        )
...
def _deduplicate_signatures(signatures: list[ExtractedSignature]) -> list[ExtractedSignature]:
    """按参数四元组键去重签名列表。"""
...
def _extract_PyMethodDef_ml_flags(field_cursor: Cursor) -> list[str]:
    ...
    return _unique_keep_order(flags)


def _infer_return_type_from_function(func_cursor: Cursor) -> str | None:
    """
    从函数体中推断 Python 返回类型。
    ...
    all_tokens = set(_collect_identifier_literal_tokens(func_cursor))
    for macro_name, type_name in RETURN_MACRO_TYPE_MAP.items():
    ...
    for node in _walk_cursor(func_cursor):
        if node.kind != CursorKind.RETURN_STMT:
            continue
        inferred = _infer_return_type_from_return_stmt(return_stmt=node, func_cursor=func_cursor)
...
class CSignatureExtractor:
    ...
    def _extract_module_from_PyModuleDef(
        ...
        module.lookup_names.update(_build_module_lookup_names(m_name))
    ...
    def _process_PyMethodDef_array_INIT_LIST_EXPR(
        ...
        for element in init_list_expr_node.get_children():
            ...
            if extracted is None:
                continue
            _add_discovered_function(output, extracted, module_name=module_name)
    ...
    def _extract_PyMethodDef_INIT_LIST_EXPR(
        ...
        # 判断哨兵
        if ml_name_cursor is None or _is_nullptr_or_zero(ml_name_cursor):
            return None
        ...
        ml_flags = _extract_PyMethodDef_ml_flags(ml_flags_cursor)
        ...
        return_type_name = _infer_return_type_from_function(function_cursor)
        if not signatures:
            # 解析不到 PyArg_* 调用时，回退到 C 形参声明推断。
            fallback = _signature_from_param_decls(function_cursor)
        ...
        signatures = [_merge_signature_return_type(sig, return_type_name) for sig in signatures]
        signatures = [_apply_method_flags(sig, ml_flags) for sig in signatures]
        signatures = _deduplicate_signatures(signatures)
    ...
    def _extract_signatures_from_function(self, func_cursor: Cursor, meth_flags: list[str]) -> list[ExtractedSignature]:
        ...
        for token_list in self._collect_pyarg_token_lists(func_cursor):
            args = _set_token_params(func_cursor, meth_flags, token_list)
            if args is not None:
                signatures.append(ExtractedSignature(arguments=args))
        ...
        for node in _walk_cursor(func_cursor):
            if node.kind == decl_stmt:
                signatures.extend(_extract_parser_signatures(node))
        return _deduplicate_signatures(signatures)
    ...
    def _collect_pyarg_token_lists(self, node: Cursor) -> list[list[str]]:
        ...
        for child in node.get_children():
            ...
            if child.kind == CursorKind.CALL_EXPR:
                token_list = _collect_call_tokens(child)
                ...
                if first_child is not None and first_child.kind == CursorKind.UNEXPOSED_EXPR:
                    token_list = _collect_call_tokens(first_child)
                else:
                    token_list = _collect_call_tokens(child)

            if token_list and _is_parameter_parser_call(token_list[0]):
